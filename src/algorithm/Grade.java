...
public class Grade {
    ...
    public Grade(ArrayList<BigDecimal> y) {
        data = y;
        n = data.size() / 2;
    }
    ...
    public ArrayList<BigDecimal> solve() {
        ...
        for (int i = 0; i < data.size(); i++) {
            if (i % 2 == 0) {
                if ((data.get(i).compareTo(new BigDecimal(0)) > 0) && (data.get(i + 1).compareTo(new BigDecimal(0)) > 0)) {
                    x = x.add(new BigDecimal(Math.log(data.get(i).doubleValue())));
                    x2 = x2.add(new BigDecimal(Math.log(data.get(i).doubleValue())).pow(2));
                    xy = xy.add(new BigDecimal(Math.log(data.get(i).doubleValue())).multiply(new BigDecimal(Math.log(data.get(i + 1).doubleValue()))));
                }
            } else if (i % 2 == 1) {
